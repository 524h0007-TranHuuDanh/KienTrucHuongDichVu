...
### T-02 — Mọi lỗi bất ngờ đều thành mã 400 `[Trung bình]`

**Vị trí:** `tuition-service/src/main/java/com/tdtu/ibanking/tuition/config/GlobalExceptionHandler.java`

Xử lý đúng 2 lỗi nghiệp vụ (404 không tìm thấy, 409 đã đóng rồi), nhưng còn nhánh `@ExceptionHandler(RuntimeException.class)` bắt **mọi lỗi lúc chạy** rồi trả 400. Lỗi tranh chấp ghi đồng thời (`ObjectOptimisticLockingFailureException`) cũng ra 400 → payment-service không hiểu, thử lại 3 lần rồi treo giao dịch ở trạng thái `PROCESSING`. **Đây là nguyên nhân gián tiếp của lỗi P-05 bên payment-service.** Ngoài ra nội dung lỗi kỹ thuật bị trả nguyên ra ngoài cho người gọi.

**Sửa:** nhánh chung → 500 + thông báo chung chung, chi tiết ghi log. Bắt riêng lỗi tranh chấp → trả 409 để payment-service hiểu là "đã có người khác đóng".
...
### T-04 — Dữ liệu mẫu không đặt lại được `[Nhẹ nhưng ảnh hưởng demo]`

**Vị trí:** `tuition-service/src/main/resources/data.sql` + `application.yml` (`spring.sql.init.mode: always`)

Chạy mỗi lần khởi động, mọi câu lệnh đều `ON CONFLICT (id) DO NOTHING`. Sau lần test đầu, học phí của `524H0088` đã `paid = true` và **không bao giờ về lại**. Muốn test lại phải xoá volume Postgres → mất luôn dữ liệu của auth-service và payment-service.

**Rủi ro thật:** chạy thử một lần trước buổi demo là hết dữ liệu để demo.

**Sửa:** đổi sang `ON CONFLICT (id) DO UPDATE SET ...` cho phần `tuitions` để mỗi lần khởi động là về trạng thái ban đầu. Hoặc thêm vài MSSV dự phòng chưa dùng tới.
...
1. **Xong T-01** (B4 → B7) — đang dở, làm nốt cho trọn
2. **A-01** — 5 phút, xoá code, mà mức độ nghiêm trọng nhất toàn dự án
3. **T-02** — vì nó là điều kiện cần để sửa P-05 và T-03
4. **T-04** — làm sớm kẻo mất dữ liệu demo
